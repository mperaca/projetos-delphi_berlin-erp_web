unit uContent.ListaOrcamentosCFOPS;
...
type
  TContentListaOrcamentosCFOPS = class(TContentBase)
  private
    function CarregaOrcamentosCFOPSDaAPI(XSearch,XOrder,XDir: string; XLimit,XOffset: integer): TFDMemTable;
...
var wcontent: TContentListaOrcamentosCFOPS;

{ TContentClientes }

constructor TContentListaOrcamentosCFOPS.Create;
begin
  inherited;
  mFileMustExist:=False;
end;

function TContentListaOrcamentosCFOPS.CarregaOrcamentosCFOPSDaAPI(XSearch,XOrder,XDir: string; XLimit,XOffset: integer): TFDMemTable;
...
//    wIdHTTP.Request.CustomHeaders.AddValue('Content-Type','application/json');
    ...
    if XLimit>0 then
       begin
         ...
         wURL := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/cfops'+wpaginacao;
       end
    else
       begin
         if Length(trim(XSearch))>0 then
            begin
              wURL  := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/cfops?'+XOrder+'='+XSearch;
              if Length(trim(XOrder))>0 then
                 wURL  := wURL+'&order='+XOrder+'&dir='+XDir;
            end
         else
            begin
              wURL  := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/cfops';
              if Length(trim(XOrder))>0 then
                 wURL  := wURL+'?order='+XOrder+'&dir='+XDir;
            end;
       end;
...
function TContentListaOrcamentosCFOPS.Execute(aRequest: THttpRequest; aReply: THttpReply;
...
begin
  try
    ...
    wmtCFOP := CarregaOrcamentosCFOPSDaAPI(wsearch,worder,wdir,wlength,wstart);
