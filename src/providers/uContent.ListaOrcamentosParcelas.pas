unit uContent.ListaOrcamentosParcelas;
...
type
  TContentListaOrcamentosParcelas = class(TContentBase)
  private
    function CarregaOrcamentosParcelasDaAPI(XSearch,XOrder,XDir: string; XLimit,XOffset: integer): TFDMemTable;
...
var wcontent: TContentListaOrcamentosParcelas;
    wnumparcelas: integer;

{ TContentClientes }

constructor TContentListaOrcamentosParcelas.Create;
begin
  inherited;
  mFileMustExist:=False;
end;

function TContentListaOrcamentosParcelas.CarregaOrcamentosParcelasDaAPI(XSearch,XOrder,XDir: string; XLimit,XOffset: integer): TFDMemTable;
...
//    wIdHTTP.Request.CustomHeaders.AddValue('Content-Type','application/json');
    ...
    if XOffset=0 then // primeira pesquisa
       begin
         if Length(trim(XSearch))>0 then
            wURL := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/parcelas/total?'+XOrder+'='+XSearch
         else
            wURL := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/parcelas/total';
    ...
    if XLimit>0 then
       begin
         ...
         wURL := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/parcelas'+wpaginacao;
       end
    else
       begin
         if Length(trim(XSearch))>0 then
            begin
              wURL  := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/parcelas?'+XOrder+'='+XSearch;
              if Length(trim(XOrder))>0 then
                 wURL  := wURL+'&order='+XOrder+'&dir='+XDir;
            end
         else
            begin
              wURL  := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/parcelas';
              if Length(trim(XOrder))>0 then
                 wURL  := wURL+'?order='+XOrder+'&dir='+XDir;
            end;
       end;
...
function TContentListaOrcamentosParcelas.Execute(aRequest: THttpRequest; aReply: THttpReply;
...
begin
  try
    ...
    case strtointdef(wordercolumn,0) of
      ...
      5: worder  := 'valordespesa';
      6: worder  := 'valordesconto';
      7: worder  := 'valorfinal';
    else
      worder := '';
    end;
    wmtParcela := CarregaOrcamentosParcelasDaAPI(wsearch,worder,wdir,wlength,wstart);
    if wmtParcela.RecordCount>0 then
       begin
         ...
         begin
           if wmtParcela.FieldByName('situacao').asString='Q' then
              wsituacao := 'Quitado'
           else if wmtParcela.FieldByName('situacao').asString='E' then
              wsituacao := 'Em Aberto'
           else if wmtParcela.FieldByName('situacao').asString='C' then
              wsituacao := 'Cancelado';


           wresult := wresult + '['+
                               '"'+wmtParcela.FieldByName('numero').asString+'", '+
                               '"'+FormatDateTime('dd/mm/yyyy',UserSession.RetornaData(wmtParcela.FieldByName('vencimento').asString))+'", '+
                               '"'+wsituacao+'", '+
                               '"'+FormatDateTime('dd/mm/yyyy',UserSession.RetornaData(wmtParcela.FieldByName('operacao').asString))+'", '+
                               '"'+FormatFloat('#,0.00',strtofloatdef(wmtParcela.FieldByName('valororiginal').asString,0))+'", '+
                               '"'+FormatFloat('#,0.00',strtofloatdef(wmtParcela.FieldByName('valordespesa').asString,0))+'", '+
