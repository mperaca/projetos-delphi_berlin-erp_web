unit uContent.ListaOrcamentosItens;
...
type
  TContentListaOrcamentosItens = class(TContentBase)
  private
    function CarregaOrcamentosItensDaAPI(XSearch,XOrder,XDir: string; XLimit,XOffset: integer): TFDMemTable;
...
var wcontent: TContentListaOrcamentosItens;
    wnumitens: integer;

{ TContentClientes }

constructor TContentListaOrcamentosItens.Create;
begin
  inherited;
  mFileMustExist:=False;
end;

function TContentListaOrcamentosItens.CarregaOrcamentosItensDaAPI(XSearch,XOrder,XDir: string; XLimit,XOffset: integer): TFDMemTable;
...
//    wIdHTTP.Request.CustomHeaders.AddValue('Content-Type','application/json');
    ...
    if XOffset=0 then // primeira pesquisa
       begin
         if Length(trim(XSearch))>0 then
            wURL := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/itens/total?'+XOrder+'='+XSearch
         else
            wURL := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/itens/total';
    ...
    if XLimit>0 then
       begin
         ...
         wURL := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/itens'+wpaginacao;
       end
    else
       begin
         if Length(trim(XSearch))>0 then
            begin
              wURL  := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/itens?'+XOrder+'='+XSearch;
              if Length(trim(XOrder))>0 then
                 wURL  := wURL+'&order='+XOrder+'&dir='+XDir;
            end
         else
            begin
              wURL  := warqini.ReadString('Geral','URL','')+'/movimentos/orcamentos/'+inttostr(UserSession.FIdOrcamento)+'/itens';
              if Length(trim(XOrder))>0 then
                 wURL  := wURL+'?order='+XOrder+'&dir='+XDir;
            end;
       end;
...
function TContentListaOrcamentosItens.Execute(aRequest: THttpRequest; aReply: THttpReply;
...
begin
  try
    ...
    wmtItem := CarregaOrcamentosItensDaAPI(wsearch,worder,wdir,wlength,wstart);
    UserSession.FMemTableOrcamentoItem := wmtItem;
