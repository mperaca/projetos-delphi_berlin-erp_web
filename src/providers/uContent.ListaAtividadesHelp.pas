unit uContent.ListaAtividadesHelp;
...
type
  TContentListaAtividadesHelp = class(TContentBase)
  private
...
var wcontent: TContentListaAtividadesHelp;
    wnumatividades: integer;

{ TContentClientes }

constructor TContentListaAtividadesHelp.Create;
begin
  inherited;
  mFileMustExist:=False;
end;

function TContentListaAtividadesHelp.CarregaAtividadesDaAPI(XSearch,XOrder,XDir: string; XLimit,XOffset: integer): TFDMemTable;
var wIdSSLIOHandlerSocketOpenSSL: TIdSSLIOHandlerSocketOpenSSL;
    wIdHTTP: TIdHTTP;
    wpaginacao,wURL,wretjson: string;
    JObj: TJSONArray;
    JObj2: TJSONObject;
    wcta: integer;
...
function TContentListaAtividadesHelp.Execute(aRequest: THttpRequest; aReply: THttpReply;
...
begin
  try
    ...
    if UserSession.FTipoHelpAtividade='pessoa' then
       begin
         if ((wdraw=1) or (wsearch='')) and (UserSession.FAtividade<>'') then
            wsearch := UserSession.FAtividade;
       end
    else if UserSession.FTipoHelpAtividade='conjuge' then
       begin
         if ((wdraw=1) or (wsearch='')) and (UserSession.FCargoConjuge<>'') then
            wsearch := UserSession.FCargoConjuge;
       end;
...
// carrega a lista de atividades da TrabinAPI
    ...
    UserSession.FNumAtividades := wmtAtividade.RecordCount;
       ...
       begin
         ...
         begin
           wresult := wresult + '['+
                               ...
                               '"<center><span class=\"fas fa-share\" align=\"middle\" title=\"Seleciona\" onclick=\"selecionaAtividade('+QuotedStr(wmtAtividade.FieldByName('id').AsString)+','+QuotedStr(wmtAtividade.FieldByName('nome').AsString)+');\"></span></center>" '+
                               '],';
           wmtAtividade.Next;
         end;
