unit uContent.ListaLocalidadesHelp;
...
type
  TContentListaLocalidadesHelp = class(TContentBase)
  private
...
var wcontent: TContentListaLocalidadesHelp;
    wnumlocalidades: integer;

{ TContentClientes }

constructor TContentListaLocalidadesHelp.Create;
begin
  inherited;
  mFileMustExist:=False;
end;

function TContentListaLocalidadesHelp.CarregaLocalidadesDaAPI(XSearch,XOrder,XDir: string; XLimit,XOffset: integer): TFDMemTable;
...
function TContentListaLocalidadesHelp.Execute(aRequest: THttpRequest; aReply: THttpReply;
...
begin
  try
    ...
    if UserSession.FTipoHelpLocalidade='pessoa' then
       begin
         if ((wdraw=1) or (wsearch=''))  and (UserSession.FLocalidade<>'') then
            wsearch := UserSession.FLocalidade;
       end
    else if UserSession.FTipoHelpLocalidade='cobranca' then
       begin
         if ((wdraw=1) or (wsearch=''))  and (UserSession.FLocalidadeCobranca<>'') then
            wsearch := UserSession.FLocalidadeCobranca;
       end
    else if UserSession.FTipoHelpLocalidade='naturalidade' then
       begin
         if ((wdraw=1) or (wsearch=''))  and (UserSession.FNaturalidade<>'') then
            wsearch := UserSession.FNaturalidade;
       end
    else if UserSession.FTipoHelpLocalidade='fiador' then
       begin
         if ((wdraw=1) or (wsearch=''))  and (UserSession.FLocalidadeFiador<>'') then
            wsearch := UserSession.FLocalidadeFiador;
       end;
    ...
    if wmtLocalidade.RecordCount>0 then
       begin
         ...
         begin
           wnomecidade := wmtLocalidade.FieldByName('nome').asString+' - '+wmtLocalidade.FieldByName('uf').asString;
           wresult := wresult + '['+
                               ...
                               '"<center><span class=\"fas fa-share\" align=\"middle\" title=\"Seleciona\" onclick=\"selecionaLocalidade('+QuotedStr(wmtLocalidade.FieldByName('id').asString)+','+QuotedStr(wnomecidade)+');\"></span></center>" '+
                               '],';
           wmtLocalidade.Next;
         end;
